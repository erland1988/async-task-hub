<template>
	<el-form ref="formRef" :model="form" :rules="rules" :label-width="options.labelWidth">
		<el-row>
          ...
          :value-format="item.format" @change="handleDatetimeChange(item.prop)"></el-date-picker>
          <el-date-picker v-else-if="item.type === 'time'" type="time" v-model="form[item.prop]"
          :value-format="item.format"></el-date-picker>
...
const handleDatetimeChange = (prop: string) => {
  ...
  if (value) {
    ...
    form.value[prop] = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`;
  }
};
</script>
